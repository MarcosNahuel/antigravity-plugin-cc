...
- **`/agy:video <file|URL> [focus]`** — *watch* a video and get a structured **visual** breakdown:
  scene-by-scene segments with timestamps, on-screen text/OCR (slides, charts, UI), and key moments —
  for screencasts, tutorials, demos, inspections. Saved to `docs/agy/video/`.
